...
const ChatPage = () => {
  ...
  const getUserChats = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/user_chatrooms/${user.id}`
      );
      if (!response.ok) {
        throw new Error(
  ...
  const getMessages = async () => {
    try {
      const response = await fetch("http://localhost:5000/chats/messages", {
        method: "POST",
  ...
  const storeMessageOnDb = async () => {
    const responce = await fetch("http://localhost:5000/messages", {
      method: "POST",
      body: JSON.stringify({
